...
firstHalf str = take halfBound str
  where
    halfBound = len `div` 2 + len `mod` 2
    len = length str
...
capitalize str = unwords capitalized
  where
    capitalized = map (\(x : xs) -> toUpper x : xs) splitWords
