...
balance db name = do
  res <- query db balanceQuery [name] :: IO [[Int]]
...
parseCommand [command, name]
  | T.unpack command == "balance" = Just (Balance name)
parseCommand [command, name, amount]
  | T.unpack command == "deposit" = do
      amt <- parseInt amount
      Just (Deposit name amt)
  | T.unpack command == "withdraw" = do
      amt <- parseInt amount
      Just (Withdraw name amt)
...
perform db (Just (Deposit name amt)) = deposit db name amt >> return (T.pack "OK")
perform db (Just (Withdraw name amt)) = deposit db name (-amt) >> return (T.pack "OK")
perform db (Just (Balance name)) = balance db name >>= \b -> return (T.pack (show b))
...
simpleServer request respond = respond (responseLBS status200 [] (BLC.pack "BANK"))
