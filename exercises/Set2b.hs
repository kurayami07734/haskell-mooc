...
countdown n = "Ready! " ++ nums ++ "Liftoff!"
  where
    nums = concatMap ((++ "... ") . show) [n, (n - 1) .. 1]
...
smallestDivisor x = fromJust $ find (\y -> x `mod` y == 0) [2 .. x]
...
biggestPrimeAtMost x = fromJust $ find isPrime [x, (x - 1) .. 2]
