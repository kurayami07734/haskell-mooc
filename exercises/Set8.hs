...
combine transform (Picture pixel1) (Picture pixel2) = Picture pixel'
  where
    pixel' coord = transform (pixel1 coord) (pixel2 coord)
...
inBounds x lo hi = x >= lo && x < hi

rectangle :: Int -> Int -> Int -> Int -> Shape
rectangle x0 y0 w h = Shape bounds
  where
    bounds (Coord x y) = inBounds x x0 (x0 + w) && inBounds y y0 (y0 + h)
...
union (Shape bound1) (Shape bound2) = Shape bound'
  where
    bound' coord = bound1 coord || bound2 coord

cut :: Shape -> Shape -> Shape
cut (Shape bound1) (Shape bound2) = Shape bound'
  where
    bound' coord = bound1 coord && not (bound2 coord)
...
paintSolid color (Shape doesBelong) (Picture pixel) = Picture pixel'
  where
    pixel' coord = if doesBelong coord then color else pixel coord
...
paint (Picture p1) (Shape doesBelong) (Picture p2) = Picture p'
  where
    p' coords = if doesBelong coords then p1 coords else p2 coords
...
instance Transform Fill where
  apply (Fill color) (Picture _) = Picture (const color)
...
instance Transform Flip where
  ...
  apply FlipX (Picture p) = Picture (p . flipX)
  apply FlipY (Picture p) = Picture (p . flipY)
...
instance Transform Blur where
  apply Blur (Picture p) = Picture p'
    where
      ...
      p' (Coord x y) = div' (sum' (neighbors x y))

      div' (r, g, b) = Color (r `div` 5) (g `div` 5) (b `div` 5)

      sum' = foldr f (0, 0, 0)

      f (Color r g b) (x, y, z) = (r + x, g + y, b + z)

      neighbors :: Int -> Int -> [Color]
      neighbors x y = map (\(dx, dy) -> p (Coord (x + dx) (y + dy))) dirs
