...
longest :: (Ord a) => [[a]] -> [a]
longest = maximumBy cmp
  where
    cmp x y = compare (length x, head y) (length y, head x)
...
winner scores player1 player2 = if p1Score >= p2Score then player1 else player2
  where
...
freqs = foldr (\k m -> Map.insertWith (+) k 1 m) Map.empty
...
transfer from to amount bank =
  case (Map.lookup from bank, Map.lookup to bank) of
    (Just balanceFrom, Just _)
      | amount < 0 || balanceFrom < amount -> bank
      | otherwise -> Map.alter credit to (Map.alter debit from bank)
    _ -> bank
  where
    credit = updateBalance (+ amount)

    debit = updateBalance (\bln -> bln - amount)

    updateBalance _ Nothing = Nothing
    updateBalance f (Just bln) = Just (f bln)
...
maxIndex array = idx
  where
    (idx, _) = maximumBy (\(_, x) (_, y) -> compare x y) (assocs array)
