...
instance Eq Name where
  ...
  (==) (Name x) (Name y)
    ...
    | toLower (head x) /= toLower (head y) = False
    | otherwise = Name (tail x) == Name (tail y)
...
instance (Foldable t, Price a) => Price (t a) where
  price :: (Foldable t, Price a) => t a -> Int
...
class Cycle a where
  ...
  stepMany :: Int -> a -> a
  stepMany steps = applyN steps step
