...
statements xs ys = liftA2 combine xs ys ++ liftA2 combine' xs ys
  where
    combine x y = x ++ " is " ++ y
...
data Op = Double | Negate

readOp :: String -> Maybe Op
readOp "double" = pure Double
readOp "negate" = pure Negate
readOp _ = Nothing

calculator :: String -> String -> Maybe Int
calculator op num = liftA2 applyOp (readOp op) (readMaybe num)
  where
    applyOp Negate x = negate x
    applyOp Double x = x * 2
...
validateDiv x y = div x <$> check' y
  where
    check' b = check (b /= 0) "Division by zero!" b
...
twoPersons name1 age1 employed1 name2 age2 employed2 =
  liftA2 (++) (fmap (: []) (liftA3 Person name1 age1 employed1)) (fmap (: []) (liftA3 Person name2 age2 employed2))
...
boolOrInt str = checkBool <|> checkInt
  where
    ...
    checkBool = check (isJust (readMaybe str :: Maybe Bool)) "Not a Bool" (Left (read str :: Bool))

    checkInt :: Validation (Either Bool Int)
    checkInt = check (isJust (readMaybe str :: Maybe Int)) "Not an Int" (Right (read str :: Int))
...
normalizePhone num = checkLength stripped *> traverse check' stripped
  where
    stripped = filter (not . isSpace) num

    checkLength str = check (length str <= 10) "Too long" str

    check' ch = check (isDigit ch) ("Invalid character: " ++ [ch]) ch
