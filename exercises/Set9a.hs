...
workload nExercises hoursPerExercise
  | timeNeeded > 100 = "Holy moly!"
  | timeNeeded < 10 = "Piece of cake!"
  | otherwise = "Ok."
  where
    timeNeeded = nExercises * hoursPerExercise
...
echo :: String -> String
echo str = intercalate ", " [drop i str | i <- [0 .. (length str)]]
...
sumSuccess xs = case rights xs of
  [] -> Left "no data"
  rs -> Right (sum rs)
...
changeCode new (Opened code) = Opened new
changeCode _ l = l
...
instance Eq Text where
  (==) :: Text -> Text -> Bool
  (==) (Text t1) (Text t2) = chars t1 == chars t2
    where
...
compose [] _ = []
compose _ [] = []
compose ((a, b) : xs) ys = case find (\(u, v) -> b == u) ys of
  Just (u, v) -> (a, v) : compose xs ys
...
permute indices xs = ys
  where
    (_, ys) = unzip (sortBy ordering' (zip indices xs))
    ordering' (idx1, _) (idx2, _) = compare idx1 idx2
